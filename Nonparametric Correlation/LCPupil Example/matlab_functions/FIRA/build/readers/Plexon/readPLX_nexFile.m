...
for i=1:nvar
    ...
    disp(sprintf('NEX: collecting %s, type %d', names(i,1:10), types(i)))
    ...
    elseif types(i) == 5 && keep_analog >= 1
        %%%
        % ANALOG CHANNEL
        %%%
        %
        ...
        if verify(FIRA.spm.analog, names(i,:))
...
if keep_analog
    ...
    if ~isempty(ks)
        ...
        if size(FIRA.analog.name, 2) ~= length(ks)
            ...
            % check if use 'default' names, otherwise take what you got
            if all(strncmp(ks, 'AD', 2))
                nums = nans(size(ks));
                for ii = 1:length(ks)
                    nums(ii) = sscanf(ks{ii}, 'AD%f');
                end
                FIRA.analog.name = getPLX_analogNames(nums);
            else
                FIRA.analog.name = ks;
            end
        end
