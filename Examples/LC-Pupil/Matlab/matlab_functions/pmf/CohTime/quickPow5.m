function vals_ = quickPow5(fits, data)
% function vals_ = quickPow5(fits, data)
% 
...
if nargin < 2 || isempty(fits)
    ...
    q2    = quick2([],data(:,[1 3]));
    vals_ = [q2(1,:); -0.5 -1.5 1.5; q2(2,:); 0 -10 10; 0 0 0.18];
    ...
    if nargin > 1 && ~isempty(data)
        ...
        if length(unique(data(:,1))) == size(data, 1)
            vals_(5,1) = min([vals_(5,3), 1.0 - data(Lmax, 2)]);
        else
            vals_(5,1) = min([vals_(5,3), 1.0 - sum(data(Lmax,2))./sum(Lmax)]);
        end
    end
else

    % compute the Quick function from the given parameters
    vals_ = 0.5 + (0.5 - fits(5)) .* (1 - exp(-(data(:,1)./...
        (fits(1).*data(:,2).^fits(2))).^(fits(3).*data(:,2).^fits(4))));
end
