function scatterhist(x,y,nbins)
...
xlabel('x'); ylabel('y');
subplot(2,2,4); bar(cx,-nx,1); h2 = gca; axis([xlim -max(nx) 0]); axis('off');
subplot(2,2,1); barh(cy-yoff,-ny,1); h3 = gca; axis([-max(ny) 0 ylim-yoff]); axis('off');
