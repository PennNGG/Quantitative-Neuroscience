function error_ = readTDL_logFile(filename)
% function error_ = readTDL_logFile(filename)
%
% Reads raw data from a file containing a "topsDataLog"
%   struct (see Tower-of-psych) and puts the data into the global
%  FIRA data structure, as follows:
%
%    FIRA.header.filename ... filename
%    FIRA.header.filetype ... 'tdl'
...
%
% history:
% created 10/20/10 by jig from readPLX_nexFile
...
for i=1:nvar
    ...
    elseif types(i) == 5 && keep_analog > 0
        ...
        %%%
        %       
        % parse channel number (1-16) from name 
        ch = sscanf(names(i,:), 'AD%f');
        if verify(FIRA.spm.analog, ch)

            % keep track of the channels we're keepinng
            ks = [ks ch];
            ...
            FIRA.raw.analog.data(start_ind:end, keep_analog+1) = ...
                fread(fid, [nPts(i) 1], 'int16').*ADtoMVs(i).*10.2;
