function vals_ = ddExp5RL_LF(fits, data, lapse, chc)
% function vals_ = ddExp5RL_LF(fits, data, lapse, chc)
%
% 6 parameters for a REINFORCEMENT LEARNING model of bias:
%   A, alpha and three terms to control the RL rule
%
...
if nargin < 2 || isempty(fits)
    ...
    if nargin < 4 || isempty(chc)
        Bs   = zeros(size(data,1),1);
    else
        % compute bias
        filt = fits(3).^(0:length(chc)-1)';
        Bs   = fits(4)*filter(filt,1,chc(:,1)) + fits(5)*filter(filt,1,chc(:,2));
        Bs   = [0; Bs(1:end-1)];
    end
