function varargout = plotGUI_raster(varargin)
...
handles.select = gsGUI_selectTrials('initf', ...
    handles.selectbutton, @plotGUI_raster);
...
% and set a max
[Lsb, uniques] = gsGUI_selectByUniqueID('getf', handles.sbmenu, trials, 16);
...
if num_rast ~= length(handles.rasteraxes)          || ...
    ...
    if num_rast > 0
        ...
        %   1. the raster data
        %   2. line at 0
        %   3. marker at raster on
        %   4. marker at raster off
        %   5. marker at rate off
        %   6. m1 (optional, from gui)
        %   7. m2 (optional, from gui)
        handles.rasterhs = zeros(num_rast, 7);
    end
            ...
            for left = br(1)+swid*[0:rc(2,num_axes)-1]
                ...
                if ind <= num_axes
                    ...
                    if num_rast > 0
                        ...
                        handles.rasterhs(ind, 2) = plot(0, 0, 'r-')';
                        handles.rasterhs(ind, 3) = plot(0, 0, 'b^', 'MarkerFaceColor', 'b', 'MarkerSize', 2)';
                        handles.rasterhs(ind, 4) = plot(0, 0, 'b^', 'MarkerFaceColor', 'b', 'MarkerSize', 2)';
                        handles.rasterhs(ind, 5) = plot(0, 0, 'r^', 'MarkerFaceColor', 'r', 'MarkerSize', 2)';
                        handles.rasterhs(ind, 6) = plot(0, 0, 'g^', 'MarkerFaceColor', 'g', 'MarkerSize', 2)';
                        handles.rasterhs(ind, 7) = plot(0, 0, 'c^', 'MarkerFaceColor', 'c', 'MarkerSize', 2)';
                    ...
                    if num_psth > 0
                        ...
                        set(handles.psthhs(ind,1), 'BarWidth', 1, 'FaceColor', 'k');
